...
public interface ProductVariantDetailRepository extends JpaRepository<ProductVariantDetail, String> {
        ...
        int updatePriceByVariantId(@Param("price") Double price, @Param("sale") Double sale,
        ...
        @Query(value = """
                            ...
                            JOIN pvd.memory m
                            JOIN pv.values vl
                            WHERE (:#{#trademark.isEmpty()} = true OR t.id IN :trademark)
                              AND (pvd.price BETWEEN COALESCE(:minPrice, 0) AND COALESCE(:maxPrice, 999999999))
                              AND (:#{#memory.isEmpty()} = true OR m.id IN :memory)
                              AND (:#{#usageCategoryId.isEmpty()} = true OR pv.usageCategory.id IN :usageCategoryId)
                              AND (:#{#values.isEmpty()} = true OR vl.value IN :values)
                              AND pvd.id IN (
                                SELECT MIN(pvd2.id)
        ...
        ProductVariantDetail findProductVariantDetailByProductVariantAndColorAndMemory(
                        @Param("productVariantId") String productVariantId, @Param("colorId") String colorId,
                        @Param("memoryId") String memoryId);

        // @Query("SELECT COUNT(od) FROM OrderDetail od WHERE od.productVariantDetail.id
        // = :productVariantDetailId")
        // long
        // countOrderDetailsByProductVariantDetailId(@Param("productVariantDetailId")
